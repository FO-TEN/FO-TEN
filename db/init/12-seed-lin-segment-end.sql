-- FO:TEN "1구간 종료 + 마지막 달 부족액 → 다음 달 한 번에 만회" 시연용 계정 — lin03
--
...
-- lin02(11-seed-lin-month7.sql)와 같다.
--
-- lin03 은 이 파일에서만 만드는 계정이다. 다시 적용할 때는 이 계정에 남아있던 데이터(시연 중에
-- 만든 2구간·대화 포함)를 전부 지우고(아래 DELETE) 여기 적힌 값으로 다시 채운다 —
-- 다른 계정과 product / exchange_rate 등 마스터 데이터에는 손대지 않는다.
--
-- 파일 번호가 09-fix-transaction-balance.sql 보다 뒤라서 이 파일 끝에서 lin03 의
...
--   통장 잔액 규칙·경계 조건은 lin02 와 같다 — 입국월 정착 비용 320,000 덕에 월말 잔액이
...
SET @lin03_alloc_p6 := 861427;
SET @lin03_shortfall := 100000;                               -- cycle 12 에 덜 낸 금액
SET @lin03_projected_interest := 5289795;
...
INSERT INTO member (login_id, password, name, nationality, language_code, created_at) VALUES
    ('lin03', '$2b$10$n29e29y3iQykR13fevsGX.0WxzbNMKsG24pxZWsbbOX0NG4rKwaB2', '팜 린', 'VIETNAM', 'vi',
     @lin03_start) AS newrow
...
FROM member WHERE login_id = 'lin03';
...
SELECT member_id, 1500000000, 'VND', @lin03_baseline, @lin03_target_krw, @lin03_baseline, @lin03_start
FROM member WHERE login_id = 'lin03';

INSERT INTO member_rate_condition_response (member_id, condition_code, will_meet, responded_at)
SELECT m.member_id, c.condition_code, c.will_meet, @lin03_start
FROM member m
...
SELECT member_id, @lin03_start, DATE_ADD(@lin03_start, INTERVAL 57 MONTH), 57
FROM member WHERE login_id = 'lin03';
...
WHERE m.login_id = 'lin03' AND rs.segment_no = 1;
...
SELECT sr.savings_roadmap_id, rs.segment_id,
       ...
       c.cycle_no, 'NONE', @lin03_baseline, 0,
       @lin03_baseline * (c.cycle_no - 1), @lin03_baseline * (c.cycle_no - 1),
       @lin03_baseline, @lin03_baseline, @lin03_projected_interest,
...
SELECT msp.monthly_saving_plan_id, ps.product_subscription_id,
       CASE ps.product_id WHEN 3 THEN @lin03_alloc_p3 ELSE @lin03_alloc_p6 END,
...
SELECT sr.savings_roadmap_id, rs.segment_id,
       DATE_FORMAT(DATE_ADD(sr.start_date, INTERVAL c.cycle_no - 1 MONTH), '%Y-%m-01'),
       CASE WHEN c.cycle_no = 12 THEN @lin03_baseline - @lin03_shortfall ELSE @lin03_baseline END, 0,
...
SELECT m.member_id,
       ...
       'SAVINGS_PAYMENT', 'OUT',
       CASE
           WHEN ps.product_id = 3 THEN @lin03_alloc_p3
           WHEN c.cycle_no = 12  THEN @lin03_alloc_p6 - @lin03_shortfall
           ELSE @lin03_alloc_p6
       END,
...
-- ============================================================
-- 소비내역(EXPENSE) — 항목 표 (lin02 와 동일)
-- ============================================================
DROP TEMPORARY TABLE IF EXISTS tmp_lin03_var, tmp_lin03_fixed, tmp_lin03_var_month;

CREATE TEMPORARY TABLE tmp_lin03_var (
    category VARCHAR(10), item_order INT, weight DECIMAL(4,2), item_name VARCHAR(30), d INT, hr INT
) CHARACTER SET utf8mb4;
INSERT INTO tmp_lin03_var VALUES
...
CREATE TEMPORARY TABLE tmp_lin03_var_month (
    months_ago INT, category VARCHAR(10), total INT
) CHARACTER SET utf8mb4;
INSERT INTO tmp_lin03_var_month VALUES
...
FROM member m
JOIN tmp_lin03_var_month vm ON vm.months_ago BETWEEN 1 AND 11
JOIN tmp_lin03_var t ON t.category = vm.category
WHERE m.login_id = 'lin03';
...
JOIN tmp_lin03_fixed f
WHERE m.login_id = 'lin03';

-- (b) 12개월 전 — 입국월(부분월)
SET @lin03_first_month_left := DAY(LAST_DAY(@lin03_start)) - DAY(@lin03_start);

INSERT INTO transaction_history
    (member_id, transaction_at, transaction_type, direction, amount, balance_after, category, expense_type, memo)
SELECT m.member_id,
       DATE_ADD(DATE_ADD(@lin03_start, INTERVAL FLOOR(@lin03_first_month_left * t.d / 28) DAY), INTERVAL t.hr HOUR),
       'EXPENSE', 'OUT', GREATEST(ROUND(vm.total * t.weight, -2), 100), 0, t.category, 'VARIABLE', t.item_name
FROM member m
JOIN tmp_lin03_var_month vm ON vm.months_ago = 12
JOIN tmp_lin03_var t ON t.category = vm.category
WHERE m.login_id = 'lin03';
...
INSERT INTO transaction_history
    (member_id, transaction_at, transaction_type, direction, amount, balance_after, category, expense_type, memo)
SELECT m.member_id,
       DATE_ADD(DATE_ADD(@lin03_start, INTERVAL FLOOR(@lin03_first_month_left * f.d / 28) DAY), INTERVAL f.hr HOUR),
       'EXPENSE', 'OUT', f.amount, 0, f.category, 'FIXED', f.item_name
FROM member m
JOIN tmp_lin03_fixed f
WHERE m.login_id = 'lin03';

-- 입국 정착 비용 — 입국 첫 이틀에 한 번만 나가는 지출 320,000 (lin02 와 동일, 이유는 그 파일 주석).
INSERT INTO transaction_history
    (member_id, transaction_at, transaction_type, direction, amount, balance_after, category, expense_type, memo)
SELECT m.member_id, DATE_ADD(DATE_ADD(@lin03_start, INTERVAL s.day_offset DAY), INTERVAL s.hr HOUR),
...
FROM member m
JOIN tmp_lin03_var_month vm ON vm.months_ago = 0
JOIN tmp_lin03_var t ON t.category = vm.category
WHERE m.login_id = 'lin03';
...
-- ============================================================
-- balance_after 재계산 — 09-fix-transaction-balance.sql 과 같은 산식, lin03 만.
-- ============================================================
