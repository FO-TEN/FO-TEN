...
class GoalCalculationServiceImplTest {
    ...
    @Test
    void calculate_체류기간_4년10개월이면_해제월만_제외한_57개월로_나눈다() {
        GoalCalculationOutput result = service.calculate(new GoalCalculationInput(
                BigDecimal.valueOf(57_000_000),
                BigDecimal.ZERO,
                LocalDate.of(2026, 1, 1),
                LocalDate.of(2030, 11, 1))); // 4년 10개월 = 58개월 - 해제월 1개월

        assertEquals(57, result.remainingMonths());
        assertEquals(BigDecimal.valueOf(1_000_000), result.targetBaselineAmount());
    }

    @Test
    void calculate_1개월을_제외하면_0이하가_되어도_0개월_개념을_쓰지_않고_최소_1개월로_계산한다() {
        GoalCalculationOutput result = service.calculate(new GoalCalculationInput(
                BigDecimal.valueOf(500_000),
                BigDecimal.ZERO,
                LocalDate.of(2026, 1, 1),
                LocalDate.of(2026, 2, 15))); // 1개월 - 1개월 = 0 → 최소 1개월로 보정
